...
namespace PositionKeep {
  ...
  export function minThreshold(strategy: Strategy, distance: number) {
    ...
    return (
      BaseMinimumThreshold[strategy] *
      (strategy == Strategy.Senkou ? 1.0 : courseFactor(distance))
    );
  }
...
export class RaceSolver {
  ...
  constructor(params: {
    ...
    if (
      StrategyHelpers.strategyMatches(this.horse.strategy, Strategy.Nige) ||
      this.pacer == null
    ) {
  ...
  initHills() {
    ...
    this.hillRng = this.course.slopes.map(
      (_) => new Rule30CARng(this.rng.int32(), this.rng.int32()),
    );
  ...
  step(dt: number) {
    ...
    const halfv = Math.min(
      this.currentSpeed + 0.5 * dt * this.accel,
      this.getMaxSpeed(),
    );
    const displacement =
      halfv + this.modifiers.currentSpeed.acc + this.modifiers.currentSpeed.err;
    ...
    this.currentSpeed = Math.min(
      halfv + 0.5 * dt * this.accel + this.modifiers.oneFrameAccel,
      this.getMaxSpeed(),
    );
    if (!this.startDash && this.currentSpeed < this.minSpeed) {
      ...
      this.startDash &&
      this.currentSpeed >= 0.85 * baseSpeed(this.course)
    ) {
  ...
  updatePositionKeepNonNige() {
    if (this.pos >= this.posKeepEnd) {
      ...
      const max =
        this.phase == 1
          ? min + 0.5 * (this.posKeepMaxThreshold - min)
          : this.posKeepMaxThreshold;
      this.posKeepEffectExitDistance =
        min + this.paceEffectRng.random() * (max - min);
  ...
  updateTargetSpeed() {
    ...
    if (this.isDownhillMode) {
      ...
      this.targetSpeed -=
        ((this.slopePer / 10000.0) * 200.0) / this.horse.power;
